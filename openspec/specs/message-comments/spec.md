...
The system SHALL anchor inline comments by byte offsets. When
the comment composer is opened from visual mode, the saved
...
The system SHALL apply both the focus and comment gutters to a
focused-and-commented block, with focus taking precedence. When
the currently-focused block is also a commented block, both
