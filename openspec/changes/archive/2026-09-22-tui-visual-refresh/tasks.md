...
- [x] 1.1 Add `HasComment bool` field to `render.Block` in
  `internal/render/render.go`.
- [x] 1.2 Populate `HasComment` from the `comments` slice in
  `RenderMessageWithComments` (any `c.BlockIdx == i` sets
  `blocks[i].HasComment = true`).
- [x] 1.3 Add a small unit test in `render_test.go` covering the
...
- [x] 2.1 In `internal/render/comment_render.go`, remove the
  ...
  tint behaviour.
- [x] 2.2 In `internal/render/comment_render_test.go`, remove
  ...
  via `Comment.Marker()`).
- [x] 2.3 In `internal/render/render.go`, update the godoc on
...
- [x] 3.1 In `internal/render/style.go`, set `Prefix` to the
  empty string for `H1`, `H2`, and `H3`.
- [x] 3.2 Change `H1` colour to `"#00d7d7"` (cyan `51`), keep
  bold, add underline.
- [x] 3.3 Change `H2` colour to `"#00d7d7"` (cyan `51`), keep
  bold; drop the prefix only (no underline).
- [x] 3.4 Change `H3` colour to `"#5fffff"` (softer cyan `87`),
  keep bold; drop the prefix only.
- [x] 3.5 Change `Document.Margin` from `uintPtr(2)` to
  `uintPtr(1)`.
- [x] 3.6 Add a small test in `style_test.go` that asserts the
  `StyleConfig` reflects the new prefix, colour, and margin
  values.

## 4. Model package: replace `injectBorder` with `injectGutter`

- [x] 4.1 Delete `borderStyle`, `injectBorder`, `trimLeadingVisible`,
  and `hasCommentGutter` from `model.go`.
- [x] 4.2 Add `injectGutter(rendered string) string` in `model.go`
...
- [x] 4.3 Update `refreshViewport` to call `injectGutter` instead
  of `injectBorder`.
- [x] 4.4 Update `visualModeStyle`: change `Background` from
  `lipgloss.Color("212")` to `lipgloss.Color("51")` (cyan).
- [x] 4.5 Add `internal/render/gutter_test.go` covering four
  cases: focused block (cyan), commented block (yellow),
  gap line (space), and focused + commented block (cyan wins).

## 5. Cleanup

- [x] 5.1 Delete `internal/render/border_test.go` (whole file;
  tests the removed `injectBorder` / `hasCommentGutter` /
  `trimLeadingVisible` helpers).
- [x] 5.2 Remove any other dangling references to
  `render.borderStyle` (the only known reference is the test
  fixture in `border_test.go`, which is deleted in 5.1).
- [x] 5.3 Run `task test` (or equivalent) and confirm all
  packages build and tests pass.

## 6. Visual smoke check

- [x] 6.1 Manually render a fixture markdown document with three
