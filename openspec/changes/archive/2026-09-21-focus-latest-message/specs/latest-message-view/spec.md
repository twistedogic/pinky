...
Focus pinky on the latest complete assistant message from the agent. Render
that message as markdown with terminal styling and width-aware wrap, and
let the user navigate within it by markdown structure (block, heading,
line) using vim-style keys with a current-block indicator.
